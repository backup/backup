    ...
    class Dropbox < Base
        ...
        File.open(src, 'r') do |file|
          ...
          if file.stat.size > (1024**2 * chunk_size)
            ...
            with_retries("Finish UPLOAD #{src} to #{dest}") do
              metadata = uploader.finish(dest, true)
            end
          else
            with_retries("UPLOAD #{src} to #{dest}") do
              metadata = connection.put_file(dest, file, true, nil)
            end
          end
        end
        # Merge in the local file's MD5 hash
        metadata.merge!('md5' => Syncer::Cloud::LocalFile.new(src).md5)
        # Update the cache
      ...
      def objects(prefix)
        ...
        folder_entries(prefix).each do |obj_data|
          unless obj_data[1]['is_dir']
            objects << Object.new(self, *obj_data) unless obj_data[1]['is_deleted']
          end
        end
      ...
      ##
      # Fetches the specified file's cached metadata from when it was last uploaded
      def fetch_metadata_cache(path)
      ...
      def metadata_cache_file(path)
        ...
        cache_key=Digest::SHA2.hexdigest(file_key)
