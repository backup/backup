module Backup
  module Record
    class Local < ActiveRecord::Base
      ...
      # Destroys all backups for the specified trigger from Local Server
      def self.destroy_all_backups(procedure, trigger)
        backups = Backup::Record::Local.all(:conditions => {:trigger => trigger})        
        unless backups.empty?

          backups.each do |backup|
            puts "\nDestroying backup \"#{backup.filename}\" from path \"#{backup.path}\"."
            begin
              %x{ rm #{File.join(backup.path, backup.filename)} }
              backup.destroy
            rescue
              backup.destroy
            end
