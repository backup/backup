...
== Backup Just Updated to Version 2.0.0! (And now updated to 2.1.0, be sure to use this one!)

== IMPORTANT: Backup SQLite3 Database will no longer be utilized as of 2.1.0!
Backup will no longer utilize the Backup SQLite3 Database as of version 2.1.0!
It seemed many users were having a struggle with SQLite3 and maintaining it with Backup, so I decided to get rid of it
and instead of using the self-contained SQLite3 database, Backup will now utilize the Production database of your application.
All you have to do is run "./script/generate backup_files" and then run "rake db:migrate" and you're set! No further "Backup" database management is required!

=== Refer to the Getting Started page to get up and running incredibly fast with Backup 2.1.0!
