...
module Backup
describe Storage::OpenStack do
  let(:model) { Model.new(:test_trigger, 'test label') }
  let(:storage) { Storage::OpenStack.new(model) }
  let(:s) { sequence '' }
  ...
  describe '#initialize' do
    ...
    it 'configures the storage' do
      storage = Storage::OpenStack.new(model, :my_id) do |cf|
        cf.keep       = 2
    ...
    it 'strips leading path separator' do
      storage = Storage::OpenStack.new(model) do |cf|
        cf.path = '/this/path'
      end
