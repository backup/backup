  ...
  describe '#generate:model' do
    ...
    it 'should include the correct option values' do
      options = <<-EOS.lines.to_a.map(&:strip).map {|l| l.partition(' ') }
        databases (firebird, mongodb, mysql, openldap, postgresql, redis, riak)
        ...
        notifiers (campfire, flowdock, hipchat, http_post, mail, nagios, pagerduty, prowl, pushover, slack, twitter)
      EOS
