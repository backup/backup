...
describe Encryptor::GPG do
  ...
  specify ':asymmetric mode with preloaded and imported keys' do
    ...
    expect( job.package.path.end_with?('.gpg') ).to be_truthy

    expect( decrypt_with_user(:backup01, job.package.path) ).to be_truthy
    expect( decrypt_with_user(:backup02, job.package.path) ).to be_truthy
    expect( decrypt_with_user(:backup03, job.package.path) ).to be_truthy
    expect( decrypt_with_user(:backup04, job.package.path) ).to be_truthy
  end
  ...
  specify ':asymmetric mode with missing recipient key' do
    ...
    expect( job.logger.has_warnings? ).to be_truthy
  ...
  specify ':asymmetric mode with no recipient keys' do
    ...
    expect( job.package.exist? ).to be_falsey
  ...
  specify ':symmetric mode' do
    ...
    expect( decrypt_with_passphrase('a secret', job.package.path) ).to be_truthy
